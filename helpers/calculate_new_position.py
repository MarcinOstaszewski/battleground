...
from constants.settings_constants import SPEED_MULTIPLIER, MAX_SPEED
...
def calculate_new_position(self, direction, speed):
    ...
    # Check for collision with screen edges, adjust position if necessary and lower health by speed
    if y < ROBOT_RADIUS or y > self.screen.get_height() - ROBOT_RADIUS:
        y = max(ROBOT_RADIUS, min(y, self.screen.get_height() - ROBOT_RADIUS))
        self.health -= abs(speed * 0.5)
    if x < ROBOT_RADIUS or x > self.screen.get_width() - ROBOT_RADIUS:
        x = max(ROBOT_RADIUS, min(x, self.screen.get_width() - ROBOT_RADIUS))
        self.health -= abs(speed * 0.5)
